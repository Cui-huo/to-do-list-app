...
```
   ...
   【尝试方案】← AI 定位问题根因，提出修复方案并执行
       ↓
   【用户反馈】← 用户报告修复结果（成功/失败/新的要求），这里要求保留原话，你可以加上你提炼的一句话信息。
       ↓  （若失败，重复上述循环直到成功）
   【最终方案】← 经过验证的正确解决方案
```
...
## 11. Android APK 闪崩 — FTS5 缺失 + 字体文件找不到两连击
...
应用在桌面端（pytest 177 全绿 + Kivy 正常运行）一切正常，但打包成 APK 安装到 Android 真机后两次闪崩：

**第一次崩溃**：`sqlite3.OperationalError: no such module: fts5` — 应用刚启动就在 `init_db()` 的 `CREATE VIRTUAL TABLE ... USING fts5` 崩溃。

**第二次崩溃**：`OSError: Label: File 'AlimamaDongFangDaKai.ttf' not found` — FTS5 修复后应用成功进入 main loop，但渲染第一帧时崩溃。

两次崩溃之间毫无关联，分别由平台 SQLite 编译选项和资源打包路径引起。
...
```
   【用户要求】← APK 启动闪退，adb logcat 显示 sqlite3.OperationalError: no such module: fts5
       ↓
   【尝试1 — 自定义 p4a recipe 启用 FTS5】
   - AI 提出：在 p4a 构建层给 sqlite3 recipe 加 -DSQLITE_ENABLE_FTS5
   - 用户批评："创建本地配置的方案不是社区推荐的，你再查查"
       ↓
   【用户反馈】← "那就别纠结fts5了，直接重新检索：kivy框架的我们这个应用背景下，
                   生成的apk应用出现当下问题该怎么办"
       ↓
   【尝试2 — 社区方案调研】
   - GitHub p4a 仓库搜索 FTS5：0 个 issue、0 个 PR
   - 阅读 p4a develop 分支 sqlite3 recipe 源码：确认从未加入 FTS5
   - PR #672(2016) FTS3 → #936 FTS4，FTS5 至今未合入
   - 社区共识：不在构建层折腾，应用层做能力检测+优雅降级
       ↓
   【最终方案 — LIKE 降级】
```
...
| 文件 | 修改 |
|------|------|
| `app_tool/model/database.py` | 新增 `_has_fts5` 模块级标记 + `has_fts5()` 查询函数；`init_db()` 捕获 `no such module: fts5` 错误，`_strip_fts5_from_schema()` 移除 FTS5 建表语句后重试；`fts_insert/update/delete()` 检查 `_has_fts5` 不可用时静默 no-op |
| `app_tool/controller/search_service.py` | 导入 `has_fts5`；关键词搜索 `has_fts5() and _is_ascii()` 双条件满足才走 FTS5 MATCH，否则走 LIKE |
| 全量测试 | 177 全绿，零回归 |

**效果**：桌面端继续使用 FTS5 索引搜索（性能最优），Android 端自动降级为 LIKE 全表扫描（几百条便签无感知差异）。
...
```
   【用户要求】← FTS5 修复后再次构建 APK，应用进入 main loop 后崩溃：
                  OSError: Label: File 'AlimamaDongFangDaKai.ttf' not found
       ↓
   【尝试1 — 字体注册失败时回退到 CJK 系统字体】
   - 方案：自定义字体找不到时，用 _FONT_PATH 注册同名回退
   - 用户拒绝："拒绝回退。寻求修复字体正常使用。"
       ↓
   【用户反馈】← 必须让自定义字体真正可用，不回退
       ↓
   【根因定位】
   - 字体文件路径：font/阿里妈妈东方大楷（建议先保存，再下载）/AlimamaDongFangDaKai-Regular.ttf
   - 中文目录名 + 全角括号（），CI Linux 环境 Buildozer 打包后文件静默丢失
   - os.path.isfile() 返回 False，LabelBase.register 跳过注册
   - 但 main_screen.py / note_card.py 大量硬编码默认字体为 "AlimamaDongFangDaKai"
   - Kivy 渲染时找不到该字体 → OSError 崩溃
       ↓
   【最终方案 — 纯英文路径】
```
...
| 文件 | 修改 |
|------|------|
| `app_tool/res/fonts/` | 新建目录，复制两个 TTF 文件（纯英文文件名） |
| `app_tool/ui/app.py` | 路径从 `_PROJECT_ROOT + "font/中文..."` 改为 `_CURRENT_DIR + "../res/fonts/..."` |
| 全量测试 | 177 全绿 |

**效果**：中文字体正常渲染，标题栏/功能栏/卡片标题均使用东方大楷字体。
...
同一个代码库，桌面 `pytest` 177 全绿 + Kivy 手动测试正常，打包成 APK 后连续两次崩溃。根因都是**平台能力差异**：
- 桌面 Python 自带 SQLite 编译了 FTS5，Android p4a 没有
- 桌面 Windows 文件系统容忍中文路径，Android/Linux 构建链不容忍

**教训**：APK 构建后必须真机测试，不能假设"桌面正常 = APK 正常"。CI 绿色只保证代码逻辑正确，不保证平台兼容性。

**2. 社区沉默 = 共识**

p4a 仓库 0 个 FTS5 issue、0 个 PR，不是没人遇到，而是社区共识就是"不做"。2016 年的 FTS PR 被标记 abandoned 已经说明了态度。强行在构建层 hack 只会引入维护负担。

**3. 资源文件路径必须纯 ASCII**

中文目录名 + 全角括号在 CI Linux 环境中是定时炸弹。Buildozer 不会报错，文件静默丢失，运行时才暴露。项目资源统一放 `app_tool/res/` 下，全程英文字母+数字+下划线+连字符。

### 后续待修复（同一 APK 版本）

- 首次启动布局挤在左下角 → `Window.size = (420, 740)` 在 Android 上干扰全屏
- 大部分中文不显示 → CJK 系统字体查找列表缺少 Android 路径 `/system/fonts/`

...
```
输入：note_id(内部) — 用户点击便签卡片上的置顶按钮
校验：is_pinned=0；已置顶则返回失败 Toast"该便签已置顶"（R6 幂等拒绝）
输出：(True, 更新后 Note) | (False, "错误信息字符串")
副作用：更新 Note 表（is_pinned=1, pinned_at=now）+ 卡片瞬移到手动置顶区（列表最上方）；写入日志（INFO 级别）

手动置顶区说明（R24 状态流转）：
- 触发方式：主界面便签卡片上点击置顶按钮
- 排序：按 pinned_at DESC（最近置顶在最上）
- 免疫排序按钮：排序偏好切换不影响手动置顶区顺序
- 冲突去重：便签若同时含置顶标签，仅出现在手动置顶区（手动置顶优先级高于标签置顶）
```

#### 取消手动置顶

```
输入：note_id(内部) — 点击已置顶便签的 [取消置顶] 按钮
校验：is_pinned=1
输出：(True, 更新后 Note) | (False, "错误信息字符串")
副作用：更新 Note 表（is_pinned=0, pinned_at=NULL）+ 卡片移出手动置顶区；写入日志（INFO 级别）
```
...
- [ ] 允许无标题、无标签，仅内容即可创建
- [ ] 未完成排序：手动置顶区（pinned_at DESC）→ 标签置顶区（标签选择顺序，同标签内 updated_at DESC）→ 排序偏好（created_at/updated_at DESC）；冲突去重（手动置顶优先）；前两区免疫排序按钮
